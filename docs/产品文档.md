# PlayDota2Win 产品文档

## 📋 目录

1. [项目概述](#项目概述)
2. [核心功能](#核心功能)
3. [技术架构](#技术架构)
4. [用户流程](#用户流程)
5. [API 接口](#api-接口)
6. [数据模型](#数据模型)
7. [安全机制](#安全机制)
8. [部署说明](#部署说明)
9. [开发指南](#开发指南)
10. [UI 设计规范](#ui-设计规范)
11. [常见问题](#常见问题)

---

## 项目概述

**PlayDota2Win** 是一个基于 React 19、Vite 和 Cloudflare Workers 构建的下载分发平台，
专为游戏工具/软件分发设计。项目采用可爱的动漫风格 UI 设计，提供跨平台（Windows、
macOS、Linux）的软件下载管理功能。

### 主要特点

- 🎨 **可爱动漫风格 UI** - 柔和的粉紫色渐变配色，丰富的动画效果
- 🔐 **多重安全防护** - 下载密码保护 + Cloudflare Turnstile 人机验证
- 📦 **多存储方式** - 支持外部链接、Cloudflare R2、自定义 S3 存储
- 🗂️ **分类管理** - 支持自定义分类（图标、颜色、描述）
- 📖 **配置指引** - 内置 Markdown 支持的配置指引系统
- ⚡ **边缘计算** - 基于 Cloudflare Workers 的全球分发
- 🎯 **管理后台** - 完整的管理界面，支持批量操作

---

## 核心功能

### 1. 公开下载页面 (`/` 或 `/download`)

#### 功能特性

- **多平台下载展示** - 自动识别并展示 Windows、macOS、Linux 版本
- **分类筛选** - 按分类浏览下载项（支持自定义图标和颜色）
- **下载保护**
  - 密码验证 - 用户需输入下载密码
  - Cloudflare Turnstile - 失败多次后自动触发人机验证
  - 短期令牌 - 生成 5 分钟有效期的下载令牌
- **配置指引**
  - 支持 Markdown 格式
  - 智能识别复制/打开动作
  - 弹窗展示详细指引
- **下载统计** - 实时显示总下载次数

#### UI 元素

- 可爱的吉祥物动画（悬停弹跳效果）
- 浮动星星和云朵背景装饰
- 渐变卡片设计
- 响应式布局

### 2. 管理后台 (`/admin`)

#### 认证系统

- 密码登录（JWT Token 认证）
- 失败次数保护（超过 3 次触发 Turnstile）
- Token 持久化（localStorage）

#### 分类管理

- **CRUD 操作** - 创建、编辑、删除分类
- **可视化编辑**
  - Emoji 图标选择器
  - 颜色选择器（预设颜色）
- **拖放排序** - 直观的拖放界面调整分类顺序
- **智能提示** - 删除分类时显示关联下载项数量

#### 下载项管理

- **添加下载项**
  - 平台选择（Windows/macOS/Linux）
  - 分类关联
  - 标题、描述、版本号、文件大小
  - 配置指引（支持多行文本）
  - 三种存储方式：
    - 外部链接 - 直接输入 URL
    - Cloudflare R2 - 自动上传到 R2 存储
    - 自定义 S3 - 使用预签名 URL 上传

- **编辑下载项**（新增功能）
  - 支持编辑文字信息（标题、描述、配置指引等）
  - 可修改平台、版本号、文件大小
  - 可更改分类关联
  - 不支持修改存储方式和下载链接（需重新添加）
  - 模态框弹窗编辑，实时保存
  - **技术实现细节**：
    - 使用 PUT 请求更新数据
    - 按钮使用 `type="button"` 防止默认表单提交
    - 模态框阻止事件冒泡（`stopPropagation`）
    - 编辑时保存原表单状态，取消时恢复

- **批量操作**
  - 批量选择/全选
  - 批量移动到分类
  - 批量删除

- **下载项操作**
  - 启用/禁用切换
  - 编辑（通过模态框）
  - 删除（R2 文件自动清理）

#### 实时统计

- 总下载次数显示
- 下载项数量统计
- 分类下载项计数

---

## 技术架构

### 前端技术栈

```
React 19 + React Router 8
├── TypeScript（严格模式）
├── Vite 8
├── React external store（Mumble 状态机）
├── CSS Modules（组件样式隔离）
└── 自定义组件
    ├── EmojiPicker - Emoji 选择器
    ├── ColorPicker - 颜色选择器
    └── DraggableList - 可拖放列表
```

### 后端架构

```
Cloudflare Workers
├── Cloudflare Vite Plugin - Worker 与 SPA 资源集成
├── Worker API Router - 独立 Request/Response 处理器
├── Cloudflare KV - 下载计数与迁移镜像
├── Cloudflare R2 - 规范元数据、文件、外链备份与安全计数
│   └── UPLOADS_BUCKET (downloads)
└── 环境变量
    ├── ADMIN_PASSWORD - 管理员密码
    ├── ADMIN_JWT_SECRET - JWT 签名密钥
    ├── ADMIN_SIGNING_SECRET - 下载路径签名密钥
    ├── DOWNLOAD_PASSWORD - 下载密码
    ├── TURNSTILE_SITE_KEY - Turnstile 站点密钥
    └── TURNSTILE_SECRET_KEY - Turnstile 服务端密钥
```

### 项目结构

```
playdota2win/
├── src/
│   ├── main.tsx                      # React 根入口
│   ├── router.tsx                    # /、/download、/admin
│   ├── pages/                        # 页面组件与页面 CSS Modules
│   ├── components/
│   │   ├── public/                   # 公开下载页组件
│   │   ├── admin/                    # 管理后台组件
│   │   └── mumble/                   # 文字/语音浮窗与状态面板
│   ├── hooks/                        # React hooks 与 external-store 适配
│   ├── lib/
│   │   ├── types.ts                  # TypeScript 类型定义
│   │   ├── auth.ts                   # 下载认证逻辑
│   │   ├── admin-auth.ts             # 管理员认证逻辑
│   │   ├── jwt.ts                    # JWT 工具函数
│   │   ├── turnstile.ts              # Turnstile 验证
│   │   ├── server/                   # Worker-only 存储与上传逻辑
│   │   └── mumble/                   # 框架无关 Mumble 客户端
│   └── worker-configuration.d.ts     # Cloudflare Worker 类型
├── worker/
│   ├── index.ts                      # Worker 入口与 API 路由
│   ├── http.ts                       # 请求事件/Cookie 适配
│   ├── security.ts                   # 同源校验与安全响应头
│   └── routes/
│       ├── public/                   # 公开 API handlers
│       └── admin/                    # 管理 API handlers
├── index.html                        # SPA HTML 入口
├── wrangler.jsonc                    # Wrangler 配置
├── package.json
├── tsconfig.json
└── AGENTS.md                         # 开发指南
```

---

## 用户流程

### 公开用户下载流程

```mermaid
sequenceDiagram
    participant U as 用户
    participant P as 下载页面
    participant A as API
    participant KV as KV 存储
    participant T as Turnstile

    U->>P: 访问下载页面
    P->>A: GET /api/downloads
    A->>KV: 获取下载列表
    KV-->>A: 返回启用的下载项
    A-->>P: 返回下载列表

    U->>P: 点击下载按钮
    P->>U: 显示密码弹窗
    U->>P: 输入密码

    alt 失败次数 >= 3
        P->>T: 加载 Turnstile
        U->>T: 完成人机验证
        T-->>P: 返回 Token
    end

    P->>A: POST /api/downloads/link
    A->>A: 验证密码和 Turnstile
    A->>KV: 生成下载令牌
    A-->>P: 返回下载链接
    P->>U: 触发下载
```

### 管理员操作流程

```mermaid
sequenceDiagram
    participant A as 管理员
    participant UI as 管理界面
    participant API as API
    participant KV as KV 存储
    participant R2 as R2 存储

    A->>UI: 访问 /admin
    UI->>A: 显示登录页面
    A->>UI: 输入密码
    UI->>API: POST /api/admin/auth
    API->>API: 验证密码
    API->>API: 生成 JWT Token
    API-->>UI: 返回 Token
    UI->>UI: 存储 Token 到 localStorage

    UI->>API: GET /api/admin (带 Token)
    API->>KV: 获取下载列表
    KV-->>API: 返回完整列表
    API-->>UI: 返回数据

    A->>UI: 添加下载项
    UI->>API: POST /api/admin (FormData)

    alt 存储类型 = R2
        API->>R2: 上传文件
        R2-->>API: 返回路径
    end

    API->>KV: 保存下载项
    API-->>UI: 返回成功
```

---

## API 接口

### 公开 API

#### 1. 获取下载列表

```http
GET /api/downloads
```

**响应示例**

```json
{
	"success": true,
	"data": {
		"items": [
			{
				"id": "uuid",
				"platform": "windows",
				"categoryId": "category-uuid",
				"title": "PlayDota2Win Windows 稳定版",
				"description": "适用于 Windows 10+",
				"configGuide": "# 配置步骤\n1. 下载后解压\n2. 运行 setup.exe",
				"filename": "PlayDota2Win.exe",
				"version": "v1.2.0",
				"size": "45MB",
				"storageType": "r2",
				"url": "/api/admin/download/windows/v1.2.0/PlayDota2Win.exe",
				"enabled": true,
				"createdAt": 1706000000000,
				"updatedAt": 1706000000000
			}
		],
		"downloadCount": 12580,
		"lastUpdated": 1706000000000
	}
}
```

#### 2. 获取分类列表

```http
GET /api/categories
```

**响应示例**

```json
{
	"success": true,
	"data": {
		"items": [
			{
				"id": "uuid",
				"name": "工具",
				"icon": "🔧",
				"color": "#667EEA",
				"description": "实用工具软件",
				"order": 0,
				"createdAt": 1706000000000,
				"updatedAt": 1706000000000
			}
		],
		"lastUpdated": 1706000000000
	}
}
```

#### 3. 验证下载

```http
POST /api/downloads/link
Content-Type: application/json

{
  "itemId": "uuid",
  "password": "下载密码",
  "turnstileToken": "可选的 Turnstile Token"
}
```

**成功响应**

```json
{
	"success": true,
	"data": {
		"url": "实际下载链接",
		"filename": "PlayDota2Win.exe",
		"count": 12581
	}
}
```

**失败响应（需要 Turnstile）**

```json
{
	"success": false,
	"error": "密码错误",
	"data": {
		"requireTurnstile": true,
		"siteKey": "0x4AAAAAAAA...",
		"failureCount": 3
	}
}
```

### 管理 API（需要 JWT 认证）

#### 1. 管理员登录

```http
POST /api/admin/auth
Content-Type: application/json

{
  "password": "管理员密码",
  "turnstileToken": "可选"
}
```

**成功响应**

```json
{
	"success": true,
	"data": {
		"token": "jwt-token"
	}
}
```

#### 2. 获取完整下载列表

```http
GET /api/admin
Authorization: Bearer {jwt-token}
```

#### 3. 添加下载项

```http
POST /api/admin
Authorization: Bearer {jwt-token}
Content-Type: multipart/form-data

platform: windows
title: 标题
description: 描述
configGuide: 配置指引
filename: 文件名
version: v1.0.0
size: 45MB
storageType: r2
categoryId: uuid (可选)
file: [文件数据] (r2/s3 需要)
url: https://... (link 需要)
s3Config: {...} (s3 需要)
```

#### 4. 更新下载项

```http
PUT /api/admin
Authorization: Bearer {jwt-token}
Content-Type: application/json

{
  "id": "uuid",
  "enabled": true,
  "categoryId": "新分类ID"
}
```

#### 5. 删除下载项

```http
DELETE /api/admin
Authorization: Bearer {jwt-token}
Content-Type: application/json

{
  "id": "uuid"
}
```

#### 6. 分类管理

```http
# 获取分类
GET /api/admin/categories
Authorization: Bearer {jwt-token}

# 创建分类
POST /api/admin/categories
Authorization: Bearer {jwt-token}
Content-Type: application/json
{
  "name": "工具",
  "icon": "🔧",
  "color": "#667EEA",
  "description": "描述",
  "order": 0
}

# 更新分类
PUT /api/admin/categories
Authorization: Bearer {jwt-token}
Content-Type: application/json
{
  "id": "uuid",
  "name": "新名称",
  ...
}

# 删除分类
DELETE /api/admin/categories
Authorization: Bearer {jwt-token}
Content-Type: application/json
{
  "id": "uuid"
}
```

---

## 数据模型

### DownloadItem (下载项)

```typescript
interface DownloadItem {
	id: string; // UUID
	platform: Platform; // 'windows' | 'macos' | 'linux'
	categoryId?: string; // 分类 ID（可选）
	title?: string; // 标题（用于展示）
	description?: string; // 描述
	configGuide?: string; // 配置指引（Markdown）
	filename?: string; // 文件名
	version: string; // 版本号（如 v1.2.0）
	size: string; // 文件大小（如 45MB）
	storageType: StorageType; // 'link' | 'r2' | 's3'
	url: string; // 存储 URL 或路径
	signedUrl?: string; // 短期签名链接（仅展示，不持久化）
	s3Config?: S3Config; // S3 配置（仅 s3 类型）
	createdAt: number; // 创建时间戳
	updatedAt: number; // 更新时间戳
	enabled: boolean; // 是否启用
}
```

### Category (分类)

```typescript
interface Category {
	id: string; // UUID
	name: string; // 分类名称
	icon?: string; // 图标（emoji 或 SVG）
	color?: string; // 颜色（十六进制）
	description?: string; // 描述
	order: number; // 排序顺序
	createdAt: number; // 创建时间戳
	updatedAt: number; // 更新时间戳
}
```

### S3Config (S3 配置)

```typescript
interface S3Config {
	endpoint?: string; // S3 Endpoint
	bucket?: string; // Bucket 名称
	region?: string; // 区域
	presignedUrl?: string; // 预签名上传 URL
	publicUrl?: string; // 公开下载 URL
}
```

### 存储结构

```typescript
// R2：规范元数据、托管文件、外链备份与安全失败计数
// KV：迁移镜像，以及 download_count:<item-id> 独立下载计数
// 下载 token 为绑定对象路径的无状态 HMAC，不写入 KV/R2。
```

---

## 安全机制

### 1. 下载保护

#### 密码验证

- 环境变量 `DOWNLOAD_PASSWORD` 配置统一下载密码
- 失败次数使用 R2 中按客户端散列隔离的安全计数对象（KV 仅保留兼容路径）
- 失败 3 次后强制启用 Turnstile

#### Cloudflare Turnstile

- 自动在失败多次后加载
- 服务端验证 Token 真实性
- 验证成功后重置失败计数

#### 短期令牌

- 验证成功后生成 HMAC 签名的无状态令牌，有效期 5 分钟
- 令牌同时绑定完整 R2 对象路径，不能改用于其他文件
- 服务端验证签名、有效期和对象路径，不提供独立 token 探测接口

### 2. 管理员认证

#### JWT Token

- 密码验证成功后签发 JWT
- Token 包含过期时间（1 小时），只保存在 Secure、HttpOnly、SameSite Cookie 中
- 使用 `ADMIN_JWT_SECRET` 签名
- 所有管理 API 需要验证 Token

#### 失败保护

- 失败次数使用 R2 安全计数对象记录
- 失败 3 次后强制启用 Turnstile
- 成功登录后重置计数

### 3. 文件访问控制

#### R2 文件中继

- 管理员访问：通过签名验证（`ADMIN_SIGNING_SECRET`）
- 公开下载：通过令牌验证
- 路径安全：拒绝目录穿越、绝对路径和反斜杠路径

#### 签名机制

```typescript
// 生成签名路径
const signedUrl = await signDownloadPath('/api/admin/download/path', secret);
// 格式：/api/admin/download/path?expires=timestamp&signature=hash
```

### 4. 输入验证

- 所有用户输入经过验证
- FormData 类型检查
- JSON 解析错误处理
- 文件类型、字段长度、请求体和流式大小限制（客户端与服务端）

---

## 部署说明

### 环境变量配置

在 Cloudflare Workers 控制台或 `.dev.vars` 文件中配置：

```bash
# 管理员认证
ADMIN_PASSWORD=你的管理员密码
ADMIN_JWT_SECRET=随机生成的JWT密钥
ADMIN_SIGNING_SECRET=随机生成的签名密钥

# 下载认证
DOWNLOAD_PASSWORD=你的下载密码

# Cloudflare Turnstile（可选）
TURNSTILE_SITE_KEY=你的站点密钥
TURNSTILE_SECRET_KEY=你的服务端密钥
```

### Wrangler 配置

`wrangler.jsonc` 已配置：

- KV Namespace: `APP_KV`
- R2 Bucket: `UPLOADS_BUCKET` (downloads)
- Custom Domain: 在 Cloudflare 控制台配置，真实域名不提交仓库

### 部署步骤

1. **安装依赖**

   ```bash
   npm install
   ```

2. **本地开发**

   ```bash
   npm run dev
   ```

3. **类型检查**

   ```bash
   npm run check
   ```

4. **构建**

   ```bash
   npm run build
   ```

5. **部署到 Cloudflare**
   ```bash
   npm run deploy
   ```

### KV 和 R2 设置

1. 在 Cloudflare 控制台创建：
   - KV Namespace（名称：APP_KV）
   - R2 Bucket（名称：downloads）

2. 更新 `wrangler.jsonc` 中的 ID：
   ```jsonc
   "kv_namespaces": [
     {
       "binding": "APP_KV",
       "id": "你的KV_ID"
     }
   ],
   "r2_buckets": [
     {
       "binding": "UPLOADS_BUCKET",
       "bucket_name": "downloads"
     }
   ]
   ```

---

## 开发指南

### 技术约定

#### React 19 约定

- 使用函数组件、hooks 与严格模式，不在 render 阶段执行副作用。
- 网络副作用必须支持取消，并在 `useEffect` cleanup 中释放资源。
- 框架无关的 Mumble 状态机通过 `useSyncExternalStore` 订阅。
- 组件样式使用同名 `*.module.css`；只在共享设计 token 时使用全局 CSS。
- 列表使用稳定业务 ID 作为 key，弹窗负责焦点圈闭与焦点恢复。

#### TypeScript

- 严格模式开启
- 优先使用显式类型注解
- API 响应使用 `satisfies ApiResponse<T>`

#### 代码风格

- 缩进：Tabs
- 格式化：`npm run format`（Prettier）
- 检查：`npm run lint`（ESLint）

### 常用命令

```bash
# 开发服务器
npm run dev

# 类型检查
npm run check
npm run check:watch

# 代码检查
npm run lint

# 代码格式化
npm run format

# 构建
npm run build

# 本地预览（生产构建）
npm run preview

# 部署
npm run deploy

# 生成 Cloudflare 类型
npm run cf-typegen
```

### 扩展性

系统支持以下扩展：

1. **添加新的下载存储类型**
   - 支持接入更多云存储服务（阿里云 OSS、腾讯云 COS 等）
   - 灵活的存储适配器设计

2. **自定义分类系统**
   - 支持无限层级分类（当前为单层）
   - 可扩展分类属性（标签、权重等）

3. **下载统计增强**
   - 按时间段统计下载量
   - 按平台分析用户偏好
   - 导出统计报表

---

## UI 设计规范

### 配色方案

- **主色调**
  - 紫色：`#6B4C9A`
  - 粉色：`#FF6B9D`
  - 渐变：`linear-gradient(135deg, #667eea 0%, #764ba2 100%)`

- **背景**
  - 页面背景：`linear-gradient(135deg, #fff5f7 0%, #f0e6ff 50%, #e6f0ff 100%)`
  - 卡片背景：`rgba(255, 255, 255, 0.85)` + `backdrop-filter: blur(10px)`

- **文字**
  - 标题：`#6B4C9A`
  - 正文：`#4D3A73`
  - 次要文字：`#8B7BA8`
  - 提示文字：`#A89BC4`

### 字体

- **标题**：`Fredoka`（Google Fonts）
- **正文**：`Nunito`（Google Fonts）
- **中文回退**：`PingFang SC`, `Microsoft YaHei`

### 组件样式

- **按钮**
  - 圆角：`border-radius: 12-20px`
  - 悬停：`translateY(-2px)` + 阴影增强
  - 渐变背景（主要按钮）

- **卡片**
  - 圆角：`border-radius: 16-20px`
  - 半透明白色背景
  - 模糊效果（`backdrop-filter`）
  - 阴影：`box-shadow: 0 8px 25px rgba(107, 76, 154, 0.12)`

- **动画**
  - 浮动星星：`animation: float 6s ease-in-out infinite`
  - 飘动云朵：`animation: drift 20s linear infinite`
  - 按钮弹跳：悬停时触发

---

## 常见问题

### Q: 如何修改下载项信息？

A: 在管理后台，点击任意下载项的"编辑"按钮，在弹出的模态框中修改信息后点击"保存"即可。支持修改标题、描述、配置指引、平台、版本号、文件大小和分类等信息。

### Q: 为什么编辑后保存没有反应？

A: 可能的原因：

- 网络连接问题，请检查网络状态
- 登录会话过期，请重新登录管理后台
- 必填字段（版本号、文件大小）未填写

### Q: 如何更改下载密码？

A: 下载密码在系统环境变量中配置，需要联系系统管理员修改。

### Q: 如何重置管理员密码？

A: 管理员密码在系统环境变量中配置，需要联系系统管理员修改。修改后需要重新登录。

### Q: 下载文件失败怎么办？

A: 请检查：

- 下载链接是否有效
- 文件是否已被删除
- 网络连接是否正常
- R2 存储的文件是否正常上传

### Q: 如何添加新的分类？

A: 在管理后台的"分类管理"区域，点击"添加分类"按钮，填写分类名称、选择图标和颜色，保存即可。

### Q: 如何调整分类顺序？

A: 在分类管理区域，使用鼠标拖放分类卡片即可调整顺序。调整后会自动保存。

### Q: 删除分类后，关联的下载项会怎样？

A: 删除分类时，系统会显示该分类下关联的下载项数量。删除后，这些下载项会自动移到"未分类"状态，不会被删除。

### Q: 人机验证（Turnstile）什么时候出现？

A: 当用户或管理员连续输入错误密码 3 次后，系统会自动启用 Turnstile 人机验证，防止暴力破解。

### Q: 配置指引支持什么格式？

A: 配置指引支持 Markdown 格式，可以使用标题、列表、代码块等格式化文本。同时支持识别特殊标记（如 `[复制]`、`[打开]`）来触发对应的操作。

### Q: 如何批量管理下载项？

A: 在管理后台：

1. 勾选需要操作的下载项（或使用全选）
2. 点击"批量移动到分类"或"批量删除"
3. 确认操作即可

### Q: 支持哪些平台的下载？

A: 系统支持 Windows、macOS、Linux 三个平台。每个下载项在添加时需要指定平台类型。

### Q: 如何查看下载统计？

A: 管理后台顶部会实时显示总下载次数。每次用户成功下载文件后，计数器会自动增加。

---

## 联系与支持

- **域名**: 由部署环境管理
- **技术栈**: React 19 + Vite 8 + Cloudflare Workers
- **部署平台**: Cloudflare Workers
- **版本历史**: 详见 [版本历史.md](./版本历史.md)

---

**生成时间**: 2026-01-24
**文档版本**: 2.0.0
**最后更新**: 2026-08-08，React 19 + Vite + 独立 Worker API 重写
