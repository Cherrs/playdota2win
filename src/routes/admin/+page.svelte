<script lang="ts">
	import type { DownloadItem, DownloadList, ApiResponse, Platform, StorageType, S3Config } from '$lib/types';
	...
	async function checkTurnstileRequired() {
		try {
			...
			const data: ApiResponse<{ requireTurnstile: boolean; siteKey: string; failureCount: number }> = await res.json();
	...
	function renderTurnstile() {
		...
		if (turnstileWidgetId) {
			try {
				(window as unknown as { turnstile: { remove: (id: string) => void } }).turnstile.remove(turnstileWidgetId);
			} catch { /* ignore */ }
		}
		...
		// 渲染新的 widget
		turnstileWidgetId = (window as unknown as { turnstile: { render: (el: HTMLElement, opts: unknown) => string } }).turnstile.render(container, {
	...
	async function handleLogin() {
		...
		try {
			...
			const data: ApiResponse<{ token?: string; requireTurnstile?: boolean; siteKey?: string; failureCount?: number }> = await res.json();
	...
	<div class="login-container">
		<div class="login-card">
			...
			<form class="login-form" onsubmit={(e) => { e.preventDefault(); handleLogin(); }}>
				...
				<button type="submit" class="login-btn" disabled={authLoading || (requireTurnstile && !turnstileToken)}>
					{#if authLoading}
	...
	<div class="admin-container">
		<header class="admin-header">
			...
			<div class="header-actions">
				<div class="stats">
					<span class="stat-item">📥 总下载次数: <strong>{downloadCount.toLocaleString()}</strong></span>
		...
		<section class="form-section">
			...
			<div class="form-grid">
				...
				<div class="form-group">
					...
					<input id="title" type="text" bind:value={formTitle} placeholder="例如：PlayDota2Win Windows 稳定版" />
				</div>
				
				<div class="form-group full-width">
					<label for="description">描述</label>
					<textarea id="description" bind:value={formDescription} placeholder="简短描述这个版本的特性或用途"></textarea>
				</div>
				
				<div class="form-group">
					<label for="filename">文件名</label>
					<input id="filename" type="text" bind:value={formFilename} placeholder="例如：PlayDota2Win.exe" />
				</div>
		...
		{#if formStorageType === 'link'}
			<div class="form-group full-width">
				...
				<input id="url" type="url" bind:value={formUrl} placeholder="https://example.com/download.exe" />
			</div>
		{/if}
			...
			<div class="s3-config">
				...
				<div class="form-grid">
					<div class="form-group">
						...
						<input id="s3Endpoint" type="url" bind:value={formS3Endpoint} placeholder="https://s3.example.com" />
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						...
						<input id="s3PresignedUrl" type="url" bind:value={formS3PresignedUrl} placeholder="https://...presigned-url" />
					</div>
					<div class="form-group">
						<label for="s3PublicUrl">公开下载 URL</label>
						<input id="s3PublicUrl" type="url" bind:value={formS3PublicUrl} placeholder="https://cdn.example.com/file" />
					</div>
					<div class="form-group">
					...
					<div class="download-item" class:disabled={!item.enabled}>
						...
						<div class="item-info">
							...
							<div class="item-meta">
								...
								{/if}
								<span>🔗 <a href={getAdminDownloadUrl(item)} target="_blank" rel="noopener">{item.url.slice(0, 50)}...</a></span>
							</div>
						</div>
